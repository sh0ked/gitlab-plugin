...
public class GitLabPushTrigger extends Trigger<Job<?, ?>> {
    ...
    public boolean isValidTriggerPhrase(String note) {
        final String regex = this.getNoteRegex();
    ...
    private void onCompletedPushRequest(AbstractBuild<?, ?> build, GitLabPushCause cause) {
        if(addCiMessage) {
            ...
            cause.getPushRequest().createCommitStatus(this.getDescriptor().getGitlab().instance(), status, Jenkins.getInstance().getRootUrl() + build.getUrl());
        }
    }
    ...
    private void onCompleteMergeRequest(AbstractBuild<?, ?> build, TaskListener listener, GitLabMergeCause cause) {
        ...
        if(addCiMessage) {
            ...
            cause.getMergeRequest().createCommitStatus(this.getDescriptor().getGitlab().instance(), status, Jenkins.getInstance().getRootUrl() + build.getUrl());
        }
    }
